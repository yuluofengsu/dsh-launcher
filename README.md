...
> **English**: One-click launcher / stopper plugin for the [DeepSeek Harness](https://www.npmjs.com/package/@deepseek-ai/dsh) (DSH) web UI on Windows. One-line install:
> `irm https://raw.githubusercontent.com/yuluofengsu/dsh-launcher/main/install-online.ps1 | iex`

为 [DeepSeek Harness](https://www.npmjs.com/package/@deepseek-ai/dsh)（DSH）Web 界面打造的一键启动 / 一键退出插件：双击即启动服务并打开应用窗口，退出完全静默并带反馈弹窗，内置内存防泄漏看门狗与**崩溃自动重启**。纯脚本（`bat` + `ps1` + `vbs` + `html`），零第三方依赖，**安装不需要管理员权限**。
...
**之后更新**：运行 `dsh-launcher.bat /update` 一键从本仓库更新到最新版（正在运行的服务不受影响，下次退出+启动后完全生效）。
...
- 🔁 **崩溃自动重启**：服务意外退出（崩溃）时看门狗自动拉起新实例（15 分钟内最多 3 次，防重启风暴），记录到 `restart.log`；内存泄漏导致的终止**不**自动重启，避免循环
- 🔄 **健壮性**：已处理幽灵 PID、双开竞态、启动中重复点击、服务异常退出
- 🔄 **一键自更新**：`dsh-launcher.bat /update` 从本仓库拉取最新版并覆盖更新
...
| `dsh-launcher.bat` | 启动入口（`/autostart` 无头、`/check` 自检、`/update` 自更新） |
| `dsh-stopper.bat` / `stop-dsh.vbs` | 退出入口（VBS 隐藏启动） |
| `launch-dsh.ps1` | 隐藏启动服务 + 等 HTTP 就绪 + 拉起看门狗 |
| `stop-dsh.ps1` | 可靠退出（先关看门狗 + PID/命令行匹配 + 关界面窗口）+ 反馈弹窗 + 日志 |
| `watchdog-dsh.ps1` | 守护进程（内存看门狗 + 崩溃自动重启，防重启风暴） |
| `update-dsh.ps1` | 自更新（从仓库拉取最新版覆盖） |
| `check-dsh.ps1` | 自检脚本 |
...
| 一键自更新 | `dsh-launcher.bat /update` |
| 卸载 | `uninstall.bat`（或 `uninstall.ps1 -Target <目录>`） |

运行状态与日志保存在 `%USERPROFILE%\.dsh\launcher\`：`dsh-web.pid`（服务 PID）、`dsh-web.log` / `dsh-web.err.log`（服务日志，超 1MB 自动清空）、`exit.log`（退出记录）、`memory.log`（守护记录）、`restart.log`（自动重启记录）。
...
**崩溃自动重启**：服务意外退出时，看门狗会在 2 秒后自动拉起新实例（15 分钟内最多 3 次，防重启风暴，记录在 `restart.log`）；内存泄漏导致的终止**不**自动重启（避免循环），此时手动点启动图标即可。退出快捷方式会先关闭看门狗再停服务，不会误触发重启。

## ❓ 常见问题

- **启动后界面打不开** → 看 `dsh-web.err.log`，或运行 `/check` 检查依赖
- **服务突然停止** → 看 `memory.log`（看门狗终止记录）、`restart.log`（自动重启记录）或 `dsh-web.err.log`（崩溃记录）
